...
export class PessoaComponent implements OnInit {
  ...
  EnviarFormulario(): void {
    ...
    if (pessoa.pessoaId > 0) {
      this.pessoasService.updatePessoa(pessoa).subscribe((resultado) => {
      ...
      this.pessoasService.savePessoa(pessoa).subscribe((resultado) => {
        this.visibilidadeFormulario = false;
        this.visibilidadeTabela = true;
  ...
  ExcluirPessoa(pessoaId: number){
    this.pessoasService.deletePessoa(pessoaId).subscribe(resultado => {
      this.modalRef.hide();
